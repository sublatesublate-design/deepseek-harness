...
- **W3.** `str_replace_editor` first matches `old_str` verbatim so mixed-EOL files stay unique. When that misses, it LF-normalizes file and search text, maps only the matched UTF-16 range back to raw offsets, and applies the first-4-KiB majority ending only to replacement text. Insert uses raw logical-line offsets and the same ending choice. Unedited intervals retain their original bytes, including mixed CRLF/LF boundaries and a trailing empty line.
...
- **P7.** `SubprocessSpawnSpec.sandbox` and `SubprocessTerminalSpawnSpec.sandbox` carry the per-call policy and optional session identity. A provider must enforce a confined policy or reject it. `LocalSubprocessRuntime` is the sole local confinement owner for ordinary and terminal spawns; terminal allocation validates raw argv, cancellation, and canonical cwd before wrapping. `tool-git` and `terminal-bash` pass the resolved policy. Unmarked trusted spawns stay unconfined, and E2B rejects confined policies until it implements them.
- **P11.** The desktop main renderer keeps top-level navigation only when the parsed origin equals the managed application origin. Other HTTP(S) destinations open externally, other schemes are denied, and native recovery actions require exact `dsh-desktop://retry` or `dsh-desktop://open-log` URLs.
