...
Plan mode 会注册一个单调的执行器守卫。`ToolDefinition.effect` 将所有有效调用中最强的效果分类为 `observe`、`interact`、`mutate` 或 `orchestrate`；省略元数据则视为未分类。Plan mode 激活时允许观察和交互，并在工具主体运行前拒绝修改、编排和未分类工具。完整工具目录仍保持可见，以稳定 Native schema 与 Code Mode SDK 输出。`run_code` 本身继续作为 Code Mode 传输，但每个嵌套 binding 都会以其具体工具定义重新进入守卫。

Effect 词汇是通用工具策略元数据，而不是 plan 专用名称列表。混合效果工具声明其最强的有效行为，因此 shell 不会因为某段命令文本看似只读就变为可信。未知的第一方或第三方工具会安全拒绝，直到其拥有方完成分类。沙箱与审批仍保持独立，因为三者回答的问题不同：plan 策略控制协作阶段，沙箱限制执行边界，审批则为拟执行动作取得许可。
...
**按 plan 专用名称允许列表筛选工具，或从参数推断安全性。** 不予采纳，因为 effect 属于每个工具自身，包括未来工具和 MCP 工具，而不应由每个 plan 部署维护一份列表。Plan mode 现在是通用 effect 元数据的具体消费方。按最强有效调用分类可使执行器保持确定性，并避免把 shell 文本或其他由模型控制的参数误作权限边界。
...
- 执行器测试允许观察与交互，拒绝修改、编排与未分类工具，证明 Code Mode binding 会重新进入守卫，并证明 HMR dispose 会移除守卫。
- 完整 agent-loop 集成测试让模型选择的修改型调用经过真实注册表，并将拒绝记录为普通失败工具结果，同时保持 plan mode 激活。
...
Plan 状态仍可重建，工具 schema 仍保持稳定，但如果进程在下一边界前退出，空闲状态下待生效的选择会丢失。进入或离开 plan mode 会改变提示词顺序 50 处及其后的内容。模型若忽略引导，修改型、编排型或未分类工具会被执行器拒绝；部署仍需独立配置沙箱和审批，以处理该协作阶段规则之外的执行约束与授权。
