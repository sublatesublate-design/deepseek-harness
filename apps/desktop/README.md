...
The application uses a main native window with a compact draggable title bar and a transparent always-on-top companion window, persisted normal bounds, single-instance focus, external-browser handoff for new windows, denied browser permissions, context isolation, renderer sandboxing, and no Node.js access in the page. Top-level navigation stays in the main renderer only when its parsed origin equals the managed application origin; other HTTP(S) destinations open externally, other schemes are denied, and only the exact retry and open-log recovery URLs invoke native actions. The companion is moved through a narrow preload IPC bridge so its page keeps normal pointer and click events; the main window hides its embedded copy while the standalone page renders only the whale. The two renderers synchronize the selected session over a desktop-only channel, then consume their own live session streams for status speech. The source launcher permits Electron to present a Windows GUI while keeping build and service consoles hidden. A localized startup document appears immediately while the service starts, and the windows are restored and presented again after application navigation or a second launch. Failure diagnostics distinguish an unreachable address, an incompatible service, an incomplete build, and authentication failures; the same window offers retry, a collapsible bounded output tail, and the current log location. Retry and application quit wait for the owned process tree to close before proceeding. Logs are written to `logs/desktop.log`, with the prior run retained as `logs/desktop.previous.log`, under Electron's DeepSeek Desktop user-data directory.
