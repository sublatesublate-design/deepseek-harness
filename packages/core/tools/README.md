...
- `ctx.tools.register(definition: ToolDefinition): () => void` Register a trusted typed same-process definition with a mandatory canonical `output` declaration. The layer is the calling context's scope: a plain plugin context registers globally; an agent's `agent.ctx` registers for that agent alone, shadowing a same-named global tool there. Duplicate names within one layer throw; non-native modes also reject the reserved `run_code` transport name. Missing or unsupported output declarations and a non-positive or non-finite `timeoutMs` fail at registration. Optional `effect` metadata classifies the strongest effect any valid call can have; omission remains deliberately unclassified. The optional synchronous `finalizeContent` callback is snapshotted when a call starts and may replace only final model-facing content after every pipeline outcome is normalized, including an error discovered while materializing another result field. Disposed with the calling fiber.
...
- `ToolDefinition` — `ToolSchema` + mandatory `output { schema, render, presentationMeta? }` + `execute(args, exec)`, optional final-content and presentation callbacks, optional `effect`, cooperative `timeoutMs`, and optional per-call `isConcurrencySafe(args)` classification. `ToolEffect` is `observe | interact | mutate | orchestrate`; it states the strongest effect any valid call can have, never appears in the model schema, and remains unclassified when omitted. A body returns only the canonical JSON value declared by the output schema and cooperatively stops through `exec.signal`. `finalizeContent(exec, result)` runs exactly once for every normalized result, including failures that bypass post-policy, and can replace only `content`; it must be synchronous and total.
...
Optional `effect` is also policy metadata and never enters `ToolSchema`, Native function definitions, or the generated Code Mode SDK. Use `observe` for reads, `interact` for user interaction or reporting that does not change project or external state, `mutate` for state changes or process control, and `orchestrate` for tools that invoke other capabilities. Classify mixed tools by their strongest valid call; consumers may fail closed when metadata is absent.
