...
- **按次文件策略 fail-closed**：E2B 沙箱会隔离远程执行世界，但本提供方尚未在该世界内实现 `read-only` 或 `workspace-write`。普通与终端 spawn 携带任一受限模式时，会在分配前拒绝而非静默丢弃策略；省略策略或使用 `danger-full-access` 时沿用 E2B 沙箱的现有访问权限。
