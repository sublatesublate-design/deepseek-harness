...
function installFakeKoffi(world: ComWorld): void {
  ...
  vi.doMock('koffi', () => ({
    default: {
      ...
      // koffi.view needs an external ArrayBuffer, which Electron's Node
      // runtime rejects with a native fatal error — the bindings must never
      // call it.
      view: () => { throw new Error('koffi.view is forbidden in the picker bindings') },
      register: (fn: (hwnd: unknown, lparam: unknown) => number) => { world.registered += 1; return { fn } },
      unregister: () => { world.unregistered += 1 },
      decode: (value: unknown, offsetOrType: unknown, length?: unknown): unknown => {
        if (offsetOrType === 'char16' && length === -1) return (value as FakePtr).text
