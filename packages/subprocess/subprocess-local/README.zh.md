...
- **终端进程所有权与限制**：`spawnTerminal` 在运行任何包装器前验证原始程序与取消状态。对于受限策略，它要求存在 `ctx.sandbox`，将现存 cwd 规范为真实路径，只允许工作区或平台临时目录根，然后限制确切 argv；这项检查防止 `node-pty` 在 OS runner 启动前先进入符号链接逃逸路径。它分配 `node-pty`，桥接 UTF-8 终端文本，检查当前前台进程组并向其发送信号，还会公开一项须等待的终止操作，在终止顶层 shell 前后清理后代进程。每次前台检查都会保留根进程树中的精确身份；Linux 还会在 POSIX 会话 leader 退出后枚举该会话。因此，之前观察到的 macOS 后代以及同会话 Linux 成员在重新设定父进程后仍受围栏保护，pid/start 身份则防止清理跟随 PID 复用。上层 PTY 后端负责提示符就绪、缓冲区与面向模型的操作。
