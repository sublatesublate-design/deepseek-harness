...
- An optional `sandbox` carries the caller's resolved per-call file policy and session identity. A provider must enforce a confined policy in its execution world or reject the spawn; silently ignoring it is invalid. `danger-full-access` and an omitted policy remain unconfined.
