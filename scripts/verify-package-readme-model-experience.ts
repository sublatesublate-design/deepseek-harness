...
const SENTENCE_MODEL_EXPERIENCE: Readonly<Record<string, SentenceContract>> = {
  ...
  'packages/client/ui-pet': { kind: 'none', reason: 'Browser-side visual plugin; reads session state without changing model input.' },
  ...
  'packages/host/plugin-inventory': { kind: 'none', reason: 'Host-side Loader projection and contained-plugin retry; registers nothing model-facing.' },
  'packages/boot/plugin-fault-boundary': { kind: 'none', reason: 'Host-side plugin lifecycle containment and recovery; registers nothing model-facing.' },
